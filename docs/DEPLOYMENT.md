...
The viewer logs allow/deny events with hashed subject identity and bundle ID, without claims, tokens or raw events. Forward logs to your protected logging system. Rate limits, TLS, session/egress policy and log retention belong to the deployment platform. The reference viewer is read-only; analyst writes or case administration need separately authorized tooling.
