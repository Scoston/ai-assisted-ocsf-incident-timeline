...
## AI evidence and approval operations (0.14.0)

Planning, failures, candidate verification, evidence inspection, human decisions and publication are recorded in the protected AI ledger. `ledger-backup` and restore include all action hash chains and review records; current actions are checked against their audit history before backup. Old reservations remain charged and old analyses are never silently approved. Keep independent audit-export pins outside the writable host, reconcile later decisions and provider calls before restoring service, and fence original writers. A rollback can resurrect a historical approval unless later revocations are reconciled.

Use `timeline-ai export` to retain the full acquired bundle, every event's chunk/source locator, exact request/response and human decisions. Audit packages are sensitive and unencrypted; use the organization's protected case store and retention policy. A locally recomputable hash chain does not defeat administrator rewriting. See [commands and trust boundaries](AI_EVIDENCE_AND_REVIEW.md).
