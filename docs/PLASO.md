...
All top-level parsers are explicitly selected, enabling their plugins instead of allowing an automatic operating-system preset to narrow coverage. Partitions, VSS stores and supported embedded archive types use `all`; otherwise Plaso defaults to skipping file entries inside archives. Resource limits still apply to expanded processing. `--timezone` defaults to UTC and is recorded; `--year` is an optional initial year for logs without one. Both are investigator assumptions, not corrections proven from evidence. For split disk images or formats that need companion files, acquire all segments together and use `--entry-point image.E01` within a source directory. Never submit only the first segment.
