...
| Cross-case data access | OIDC issuer/subject authorization, fixed case mapping, expiry checks, signed/pinned evidence, evidence read-only; separate case-scoped AI reviewer policy | OS access, reverse proxy, session revocation and policy deployment are platform responsibilities |
...
| AI prompt injection or fabricated conclusions | Untrusted-data instructions, no tools/actions, exact chunk/field witnesses, fixed factual templates, explicit claim-level human approval and ledger-backed presentation | Hypotheses still require human entailment assessment; matching citations cannot prove it |
...
AI audit chains detect edits relative to retained history and independent pins. They cannot defeat full database rewrites, rollback of every copy, stolen reviewer sessions or malicious administrators. The shared viewer derives reviewer identity from validated OIDC claims; trusted service APIs must not accept caller-supplied principal arguments. Exported JSON and Delta analysis releases are historical snapshots; recheck the authoritative ledger before live presentation and reconcile later revocations. [AI controls and limits](AI_EVIDENCE_AND_REVIEW.md).
