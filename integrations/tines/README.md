...
## AI evidence and human review handoff (0.14.0)

The supplied evidence-publication stories remain deterministic and do not call an AI. Before a story presents AI interpretation, an authenticated case-authorized harness service must call `timeline_demo.integrations.tines_ai.approved_handoff(harness, bundle, action_id, review_policy=path, review_policy_sha256=pin)`. It revalidates evidence and the latest human decision, then returns action/result/review hashes, full chunk IDs and an idempotency key. The helper prepares a receipt and makes no HTTP call. Provision the service endpoint and its identity through your tenant's approved deployment process.

Stop the story on missing, failed, stale or rejected approval. Human review is a separate decision, not the AI `review` task and not a boolean supplied by a webhook. Forward only the approved receipt or an authenticated link to accepted output; do not forward raw candidate prose. Polling and approval checks use zero model tokens. Recheck the live gate before delivery because an earlier approval may have been revoked. [Full workflow and audit export](../../docs/AI_EVIDENCE_AND_REVIEW.md).
