...
The Databricks source notebooks in `integrations/databricks/` use workspace `spark` and `dbutils` objects. Import them into Databricks for interactive analyst use; the deployed job uses positional wheel tasks to protect its configured policy from job parameter pushdown. These seven `.ipynb` files also work in local Jupyter. The optional OCSF job task and its strict/quarantine behavior are covered in the [OCSF guide](../docs/OCSF_EXPORT.md).
